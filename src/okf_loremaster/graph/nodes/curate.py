"""The curate node: one balanced-tier judgment per topic, then arithmetic.

The screener saw each paper alone. The curator sees a topic whole, which is the only
point in the run where "these four papers report the same result from the same cohort"
is a thing anyone can notice. That is why curation is per topic and not per paper, and
why it is worth a balanced-tier model at a few calls rather than a fast-tier one at a few hundred.

Everything after the judgment is `curation.enforce_bounds` — the ceiling, the floor, the
global target. Splitting it that way is what makes topic sizes reproducible across runs
even though the judgment behind them is not.

**A re-query round re-curates only the topics that came up short.** A gapped topic is
by definition a small one, so it is re-offered whole rather than only its new papers:
seeing the topic entire is the whole reason this node exists, and on a topic under its
floor that costs almost nothing. Topics that were already fine keep their first-round
...
# Papers offered to a topic, as a multiple of its ceiling. Past this the call is paying
...
async def curate_node(state: RunState, deps: Deps) -> dict[str, Any]:
    ...
    with span(deps, NODE) as report:
        ...
        assigned = _assign(charter, verdicts, by_pmid, query_topic)
        ...
        result = _result(placement.topics, placement.gaps, decisions)
        report["summary"] = placement.summary()

    return {"curation": result, "topics": placement.topics, "warnings": warnings}


# --- deciding what each topic is offered ------------------------------------
...
def _assign(
    ...
    """A topic for every screened paper, or nothing if none of the three sources knows.

    Three sources, in descending order of how much they read: the screener's own hint,
    which topic's query found the paper, and lexical overlap with the topic's title,
    scope and seed terms. The third exists because the broad queries in a plan carry no
    topic affinity at all, and without it every paper they alone found would be
    unofferable — a silent loss of exactly the papers a taxonomy did not anticipate.
    """
    known = set(charter.slugs)
    vectors = {topic.slug: _topic_tokens(topic) for topic in charter.topic_taxonomy}
    ...
    for pmid, verdict in verdicts.items():
        ...
        if candidate is None:
            continue
        topic = verdict.topic if verdict.topic in known else ""
        if not topic:
            found = topic_affinity(candidate, query_topic)
            topic = found if found in known else ""
        if not topic:
            topic = _nearest(candidate, charter, vectors)
        if topic:
            assigned[pmid] = topic
    return assigned


def _topic_tokens(topic: Topic) -> frozenset[str]:
...
def _nearest(
    ...
    slug order means a tie goes to the earlier topic every time rather than to whichever
...
async def _curate_all(
    ...
    """One call per topic that needs one. Returns decisions and `missing`, both by topic."""
    carried = _carried(prior)
    todo = [
        topic
        for topic in charter.topic_taxonomy
        if offers.get(topic.slug) and _needs_call(topic.slug, offers, prior, carried)
    ]
    ...
    total = len(todo)
    if total:
        deps.progress(NODE, f"curating {total} topic/topics", current=0, total=total)
    results = await asyncio.gather(
        *(
            _curate_one(deps, charter, topic, offers[topic.slug], verdicts, by_pmid, warnings)
            for topic in todo
        )
    )
    for done, (topic, (judged, note)) in enumerate(zip(todo, results, strict=True), start=1):
        deps.progress(NODE, f"curated {topic.slug}", current=done, total=total)
        decisions[topic.slug] = _reconcile(topic.slug, offers[topic.slug], judged, order)
        missing[topic.slug] = note
...
def _needs_call(
    ...
    """Whether this topic is worth a call on this round.

    Every topic on the first round. On a re-query round, only a topic that was under its
    floor *and* now has a paper it has not seen: a topic that came up short and got
    nothing new has already given its answer, and re-asking would spend a call to
    receive it again.
    """
    if prior is None:
        return True
    if not any(gap.topic == slug and gap.shortfall > 0 for gap in prior.gaps):
        return False
...
def _carried(prior: CurationResult | None) -> dict[str, list[CurationDecision]]:
    ...
    for decision in prior.decisions:
        if decision.topic:
            carried.setdefault(decision.topic, []).append(decision)
    return carried


def _carried_missing(prior: CurationResult | None) -> dict[str, str]:
    return {} if prior is None else {gap.topic: gap.missing for gap in prior.gaps}
...
async def _curate_one(
    ...
    by_pmid: Mapping[str, Candidate],
    warnings: list[str],
) -> tuple[TopicCuration | None, str]:
    """Ask about one topic. `None` when the call could not be made or read."""
    ...
    try:
        result = await deps.router.complete(
            ...
            response_format=response_format_for(TopicCuration, name="topic_curation"),
        )
        judged = parse_model(result.text, TopicCuration)
...
def _reconcile(
    ...
    lands in the reserve where a thin topic can still reach it.
    """
    if judged is None:
        # The failure path: keep the best-ranked, up to what a topic would hold anyway.
        ranked = sorted(offered, key=lambda pmid: order.get(pmid, len(order)))
        return [
            CurationDecision(pmid=pmid, keep=True, topic=slug, rationale="curation unavailable")
            for pmid in ranked
        ]
...
def _reserve(
    ...
    """The fallback queue per topic, best candidate first.

    Everything a topic could still take, tiered by how strong the "no" against it was
...
def _result(
    ...
    """One record whose `by_topic()` is the placement, not an earlier draft of it.

    The curator's decisions and the final topics disagree by design — the bounds trim
        ...
        for pmid in pmids:
            ...
            final.append(
                CurationDecision(
                    ...
                    keep=True,
                    topic=slug,
                    rationale=rationales.get(pmid, "backfilled to keep the topic above its floor"),
                )
            )
    for slug, found in decisions.items():
        for decision in found:
            if decision.pmid not in placed:
                final.append(decision.model_copy(update={"keep": False, "topic": slug}))
