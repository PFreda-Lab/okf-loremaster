"""Making the topics fit, once the curator has judged the papers.

Curation is two things, kept strictly apart. The judgment — does this paper belong on
this topic — is the model's, one call per topic. Everything in this module is the
arithmetic that follows: the ceiling, the floor, the global target, and an honest record
of what could not be filled. That split is why the size of every topic is reproducible
...
on this topic" means the same thing to the trim, the backfill and the final listing.
...
3. `target_papers` is a trim of the largest topics, and the only bound allowed to fail.
   A floor is a statement that a topic below it is not worth having at all, so when the
   floors cannot fit inside the target, the target gives way — and says so.

Nothing here names a topic, a condition, or a vocabulary. Every slug arrives from the
charter.
"""
...
from okf_loremaster.schemas import Charter, TopicGap
...
class Placement:
    """The final topics, and what the bounds cost to reach them."""

    topics: dict[str, list[str]]
    # Topics still under their floor after the backfill. Carries the curator's own
    ...
    @property
    def total(self) -> int:
        return sum(len(pmids) for pmids in self.topics.values())

    @property
    def filled(self) -> int:
        return sum(1 for pmids in self.topics.values() if pmids)

    def summary(self) -> str:
        parts = [f"{self.total} papers across {self.filled} of {len(self.topics)} topics"]
        if self.backfilled:
...
def enforce_bounds(
    ...
    *,
    ...
    `kept` is PMIDs per topic slug, as decided. `rank` is the total order — lower is
    ...
    `reserve` is the fallback queue per topic, **consumed in the order given**. The
    caller decides what "nearest miss" means; this only decides how many are taken.

    Every topic in the charter appears in the result, including the empty ones. A topic
    ...
    # PMID to the topic holding it. A paper appears on exactly one topic: the bundle
    ...
    if unknown:
        warnings.append(
            "curation named topic(s) the charter does not have, and their papers were "
            "dropped: " + ", ".join(unknown)
        )
    if duplicates:
        warnings.append(
            f"{duplicates} paper(s) were kept on more than one topic; each stayed on the "
            "first topic in the charter's order"
        )
    ...
    # takes a topic below its floor, so it cannot create or deepen a gap.
    gaps = tuple(
        TopicGap(
            topic=slug,
...
def _place(
    ...
    """Assign each kept paper to one topic. Returns how many were claimed twice.

    Iterates the charter's slug order rather than the mapping's, so a paper two topics
...
def _orderer(rank: Mapping[str, int]) -> Callable[[str], tuple[int, str]]:
    ...
    The PMID breaks ties rather than list position, so that two topics holding the same
