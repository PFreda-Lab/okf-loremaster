...
corpus, not any one test: four shelves named after the four topics, a screener that
...
from okf_loremaster.schemas import Charter, ConceptRecord, Shelf
...
SHELF_MIN, SHELF_MAX, TARGET = 4, 8, 24

# What each of the three well-served shelves keeps on the first round. Equal to the
# ceiling, so the bundle-wide target is what decides the final sizes rather than the
# curator's generosity.
KEEP_PER_SHELF = SHELF_MAX

# The two ordinary `delta` papers the screener lets through. Enough that the shelf is
# curated at all — a shelf offered nothing is never asked, and never reports a gap.
DELTA_INCLUDED = 2
...
def scripted_run(**overrides: Any) -> ScriptedLLM:
    ...
    says in as many words that its shelf lacks them — which is the only route by which
    ...
    def screen(text: str) -> dict[str, Any]:
        ...
        if topic != "delta":
            return verdict(include=True, relevance=3, shelf=topic, reason="on point")
        if is_rescue(n):
            return verdict(include=True, relevance=3, shelf="delta", reason="the rescue cohort")
        if n < DELTA_INCLUDED:
            return verdict(include=True, relevance=2, shelf="delta", reason="thin but usable")
        return verdict(include=False, relevance=1, shelf="delta", reason="not this review")

    def curate(slug: str, offered: list[str]) -> dict[str, Any]:
        if slug != "delta":
            return curation({p: index < KEEP_PER_SHELF for index, p in enumerate(offered)})
            ...
            *(
                {
                    ...
                    "rationale": f"seeds the {topic} shelf",
                    "shelf": topic,
                }
                for topic in TOPICS
            ),
...
class Run:
    ...
    @property
    def shelves(self) -> dict[str, list[str]]:
        return dict(self.state["shelves"])

    @property
    def gaps(self) -> list[str]:
        curation_result = self.state["curation"]
        assert curation_result is not None
        return [gap.shelf for gap in curation_result.gaps if gap.shortfall > 0]
    ...
    def calls_for(self, shelf: str) -> int:
        return self.scripted.curated.count(shelf)
